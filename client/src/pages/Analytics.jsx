...
export default function Analytics() {
  ...
  const exportQuestionsCsv = () => downloadCsv(
    "most-asked-questions.csv",
    ["Question", "Times asked"],
    topQuestions.map((q) => [q.question, q.count])
  );

  const exportMissedFaqsCsv = () => downloadCsv(
    "missed-faq-suggestions.csv",
    ["Question", "Suggested answer", "Times asked (30d)"],
    missedFaqs.map((m) => [m.question, m.suggestedAnswer, m.count])
  );

  const exportAgentStatsCsv = () => downloadCsv(
    "team-performance.csv",
    ["Agent", "Chats handled", "Resolved", "Avg. response time (s)"],
    agentStats.map((a) => [a.agentName, a.chatsHandled, a.resolvedCount, a.avgResponseSeconds ?? ""])
  );
          ...
          <div className="forge-card" style={s.gridCard}>
            <div style={s.leadsHeader}>
              ...
              {topQuestions.length > 0 && (
                <button className="forge-ghost" style={s.ghostBtn} onClick={exportQuestionsCsv}>Export CSV</button>
              )}
            </div>
                ...
                return (
                  <div key={l._id}>
                    <button className="forge-ghost" style={s.leadRow} onClick={() => toggleLeadConversation(l._id)}>
                      ...
                      <svg
                        style={{ ...s.leadChevron, transform: isOpen ? "rotate(180deg)" : "none" }}
          ...
          {missedFaqs.length > 0 && (
            <div className="forge-card" style={s.gridCard}>
              <div style={s.leadsHeader}>
                ...
                <button className="forge-ghost" style={s.ghostBtn} onClick={exportMissedFaqsCsv}>Export CSV</button>
              </div>
          ...
          {agentStats.length > 0 && (
            <div className="forge-card" style={s.gridCard}>
              <div style={s.leadsHeader}>
                ...
                <button className="forge-ghost" style={s.ghostBtn} onClick={exportAgentStatsCsv}>Export CSV</button>
              </div>
